...
export const fetchAeroplane = createAsyncThunk(GET_AEROPLANE, async () => {
  const data = await fetch('https://jetlog.onrender.com/api/v1/aeroplanes');
...
export const addAeroplane = (aeroplane) => (dispatch) => {
  axios
    .post('https://jetlog.onrender.com/api/v1/aeroplanes', aeroplane)
    .then((res) => {
      dispatch({
        type: ADD_AEROPLANE,
        payload: res.data,
...
export const deletePlane = createAsyncThunk(DELETE_PLANE, async (id) => {
  await fetch(`https://jetlog.onrender.com/api/v1/aeroplanes/${id}`, {
    method: 'DELETE',
  });
  return +id;
});
